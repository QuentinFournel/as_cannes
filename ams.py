...
metrics_by_position = [
    ...
    },
    {
        ...
        "metrics": {
            ...
            "Précision\npasses": "Passes précises, %",
            "xA": "xA par 90",
            "Secondes\npasses décisives": "Secondes passes décisives par 90",
            "Passes\nclés": "Passes quasi décisives par 90",
...
kpi_by_position = {
    "Buteur": {
        ...
        "Vision du jeu": {
            "xA par 90": 0.2,
            "Passes quasi décisives par 90": 0.175,
            "Secondes passes décisives par 90": 0.175,
            "Passes décisives avec tir par 90": 0.1,
            "Passes judicieuses par 90": 0.1,
            "Réalisations en profondeur par 90": 0.05,
            "Passes vers la surface de réparation par 90": 0.05,
            "Passes dans tiers adverse par 90": 0.05,
            "Passes progressives par 90": 0.05,
            "Passes pénétrantes par 90": 0.05
        },
        "Percussion": {
...
def calcul_scores_par_kpi(df, joueur, poste):
    ...
    df_scores = df_ranked[['Joueur + Information', 'Équipe dans la période sélectionnée', 'Poste', 'Âge', 'Taille', 'Passeport pays', 'Pied', 'Minutes jouées', 'Contrat expiration']].copy()
...
def plot_team_metrics(df, x_metric, y_metric):
    ...
    logos_dict = {
        ...
        "Anglet Genets": "https://upload.wikimedia.org/wikipedia/fr/8/84/Logo_Les_Gen%C3%AAts_d%27Anglet_-_2018.svg",
        "Angoulême": "https://upload.wikimedia.org/wikipedia/fr/c/c5/Angoul%C3%AAme_CFC_2020.svg",
        "Bergerac": "https://upload.wikimedia.org/wikipedia/fr/6/67/Logo_Bergerac_P%C3%A9rigord_FC.svg",
        "Cannes": "https://upload.wikimedia.org/wikipedia/fr/7/72/AS_Cannes_foot_Logo_2017.svg",
        "Fréjus St-Raphaël": "https://upload.wikimedia.org/wikipedia/fr/5/55/Logo_%C3%89FC_Fr%C3%A9jus_Saint-Rapha%C3%ABl_-_2020.svg",
        "GOAL FC": "https://upload.wikimedia.org/wikipedia/fr/d/de/Logo_GOAL_FC_-_2020.svg",
        "Grasse": "https://upload.wikimedia.org/wikipedia/fr/f/f8/Logo_RC_Pays_Grasse_2022.svg",
        "Hyères FC": "https://upload.wikimedia.org/wikipedia/fr/3/3f/Logo_Hy%C3%A8res_83_Football_Club_-_2021.svg",
        "Istres": "https://upload.wikimedia.org/wikipedia/fr/b/b0/Logo_Istres_FC_-_2022.svg",
        "Jura Sud Foot": "https://upload.wikimedia.org/wikipedia/fr/b/ba/Logo_Jura_Sud_Foot.svg",
        "Le Puy F.43 Auvergne": "https://upload.wikimedia.org/wikipedia/fr/8/88/Logo_Puy_Foot_43_Auvergne_2017.svg",
        "Marignane Gignac CB": "https://upload.wikimedia.org/wikipedia/fr/b/bb/Logo_Marignane_Gignac_C%C3%B4te_Bleue_FC_-_2022.svg",
        "Rumilly Vallières": "https://upload.wikimedia.org/wikipedia/fr/4/40/Logo_Groupement_Football_Albanais_74_-_2021.svg",
        "Saint-Priest": "https://upload.wikimedia.org/wikipedia/fr/4/46/Logo_AS_St_Priest.svg",
        "Toulon": "https://upload.wikimedia.org/wikipedia/fr/d/d6/Logo_SC_Toulon.svg",
        "St Maur Lusitanos": "https://upload.wikimedia.org/wikipedia/fr/8/89/Logo_US_Lusitanos_Saint-Maur_2018.svg",
...
def search_recommended_players(df, poste, thresholds):
    ...
    df_scores = df_ranked[['Joueur + Information', 'Âge', 'Taille', 'Passeport pays', 'Pied', 'Minutes jouées', 'Contrat expiration'] + list(thresholds.keys())].copy()
...
def _ligne_note_globale(note, percentile, note_max):
    """Bloc note globale : pleine largeur, barre épaisse, visuellement distinct des KPI."""
    p = 0.0 if pd.isna(percentile) else max(0.0, min(100.0, float(percentile)))
    coul = _couleur_pct(percentile)
    pct_txt = "&mdash;" if pd.isna(percentile) else f"{percentile:.0f}"
    note_txt = "&mdash;" if pd.isna(note) else f"{note:.1f}"
    return (
        f'<div style="background:{C_BAND};border-radius:10px;padding:14px 16px;margin-top:12px;">'
        f'  <div style="display:flex;align-items:center;justify-content:space-between;'
        f'    gap:10px;margin-bottom:8px;">'
        f'    <div style="font-size:14px;font-weight:800;letter-spacing:.4px;'
        f'      text-transform:uppercase;color:{C_INK};">Note globale</div>'
        f'    <div style="flex-shrink:0;">'
        f'      <span style="font-size:20px;font-weight:800;color:{coul};">{note_txt}</span>'
        f'      <span style="font-size:12px;color:{C_MUTE};font-weight:600;">/ {note_max:g}</span>'
        f'    </div>'
        f'  </div>'
        f'  <div style="display:flex;align-items:center;gap:10px;">'
        f'    <div style="flex:1;height:14px;border-radius:7px;background:{C_BG};'
        f'      border:1px solid rgba(61,58,42,.15);overflow:hidden;">'
        f'      <div style="width:{p:.1f}%;height:100%;background:{coul};"></div>'
        f'    </div>'
        f'    <div style="width:96px;flex-shrink:0;text-align:right;font-size:12px;'
        f'      color:{C_MUTE};">percentile <b style="color:{coul};">{pct_txt}</b></div>'
        f'  </div>'
        f'</div>'
    )
...
def construire_rating_html(df, joueur_scores, kpis, poste=None, nom_joueur=None,
    ...
    poste         : poste sélectionné (affiché dans le sous-titre)
    """
    nom_joueur = nom_joueur or str(joueur_scores.get('Joueur + Information', '')).split(' - ')[0]

    # sous-titre : club · âge · minutes · poste sélectionné
    ...
    poste = poste or joueur_scores.get('Poste', '')
    bits = [str(eq)] if eq else []
...
def afficher_rating(df, joueur_scores, kpis, poste=None, nom_joueur=None,
                    note_max=100, note_globale_kpi="Note globale"):
    st.html(construire_rating_html(df, joueur_scores, kpis, poste, nom_joueur,
    ...
    if page == "Accueil":
        ...
        journées = {
            ...
            "25-26": {
                ...
                "J18": [
                    "AS Cannes VS Andrézieux-Bouthéon FC",
                    "AS Saint-Priest VS RC Grasse",
                    "FC Rousset Sainte-Victoire VS GFA Rumilly Vallières",
                    "GOAL FC VS EFC Fréjus Saint-Raphaël",
                    "Nîmes Olympique VS SC Toulon",
                    "US Lusitanos Saint-Maur VS FC Limonest DSD",
                    "US Créteil Lusitanos VS FC 93",
                    "Hyères FC VS FC Istres"
                ],
                "J19": [
                    "Andrézieux-Bouthéon FC VS AS Saint-Priest",
                    "FC 93 VS Nîmes Olympique",
                    "GFA Rumilly Vallières VS US Lusitanos Saint-Maur",
                    "FC Limonest DSD VS Hyères FC",
                    "RC Grasse VS US Créteil Lusitanos",
                    "SC Toulon VS GOAL FC",
                    "EFC Fréjus Saint-Raphaël VS FC Rousset Sainte-Victoire",
                    "FC Istres VS AS Cannes"
                ],
                "J20": [
                    "Hyères FC VS GFA Rumilly Vallières",
                    "GOAL FC VS FC 93",
                    "Nîmes Olympique VS RC Grasse",
                    "US Créteil Lusitanos VS AS Saint-Priest",
                    "US Lusitanos Saint-Maur VS EFC Fréjus Saint-Raphaël",
                    "AS Cannes VS FC Limonest DSD",
                    "FC Rousset Sainte-Victoire VS SC Toulon",
                    "Andrézieux-Bouthéon FC VS FC Istres"
                ],
                "J21": [
                    "FC Istres VS AS Saint-Priest",
                    "GFA Rumilly Vallières VS AS Cannes",
                    "FC Limonest DSD VS Andrézieux-Bouthéon FC",
                    "RC Grasse VS GOAL FC",
                    "SC Toulon VS US Lusitanos Saint-Maur",
                    "US Créteil Lusitanos VS Nîmes Olympique",
                    "EFC Fréjus Saint-Raphaël VS Hyères FC",
                    "FC 93 VS FC Rousset Sainte-Victoire"
                ],
                "J22": [
                    "AS Saint-Priest VS Nîmes Olympique",
                    "AS Cannes VS EFC Fréjus Saint-Raphaël",
                    "Andrézieux-Bouthéon FC VS GFA Rumilly Vallières",
                    "GOAL FC VS US Créteil Lusitanos",
                    "Hyères FC VS SC Toulon",
                    "US Rousset Sainte-Victoire VS RC Grasse",
                    "US Lusitanos Saint-Maur VS FC 93",
                    "FC Istres VS FC Limonest DSD"
                ],
                "J23": [
                    "SC Toulon VS AS Cannes",
                    "Nîmes Olympique VS GOAL FC",
                    "RC Grasse VS US Lusitanos Saint-Maur",
                    "FC Limonest DSD VS AS Saint-Priest",
                    "EFC Fréjus Saint-Raphaël VS Andrézieux-Bouthéon FC",
                    "US Créteil Lusitanos VS FC Rousset Sainte-Victoire",
                    "FC 93 VS Hyères FC",
                    "GFA Rumilly Vallières VS FC Istres"
                ],
                "J24": [
                    "AS Cannes VS FC 93",
                    "AS Saint-Priest VS GOAL FC",
                    "FC Limonest DSD VS GFA Rumilly Vallières",
                    "Hyères FC VS RC Grasse",
                    "US Lusitanos Saint-Maur VS US Créteil Lusitanos",
                    "Andrézieux-Bouthéon FC VS SC Toulon",
                    "FC Istres VS EFC Fréjus Saint-Raphaël",
                    "FC Rousset Sainte-Victoire VS Nîmes Olympique"
                ],
                "J25": [
                    "RC Grasse VS AS Cannes",
                    "Nîmes Olympique VS US Lusitanos Saint-Maur",
                    "US Créteil Lusitanos VS Hyères FC",
                    "SC Toulon VS FC Istres",
                    "EFC Fréjus Saint-Raphaël VS FC Limonest DSD",
                    "GFA Rumilly Vallières VS AS Saint-Priest",
                    "FC 93 VS Andrézieux-Bouthéon FC",
                    "GOAL FC VS FC Rousset Sainte-Victoire"
                ],
                "J26": [
                    "AS Cannes VS US Créteil Lusitanos",
                    "Hyères FC VS Nîmes Olympique",
                    "US Lusitanos Saint-Maur VS GOAL FC",
                    "FC Limonest DSD VS SC Toulon",
                    "Andrézieux-Bouthéon FC VS RC Grasse",
                ...
                "J28": [
                    "AS Cannes VS GOAL FC",
                    "Andrézieux-Bouthéon FC VS Nîmes Olympique",
                    "AS Saint-Priest VS US Lusitanos Saint-Maur",
                    "GFA Rumilly Vallières VS FC 93",
                    "FC Limonest DSD VS RC Grasse",
                    "EFC Fréjus Saint-Raphaël VS SC Toulon",
                    "FC Istres VS US Créteil Lusitanos",
                    "Hyères FC VS FC Rousset Sainte-Victoire"
                ],
                "J29": [
                    "FC Rousset Sainte-Victoire VS AS Cannes",
                    "Nîmes Olympique VS FC Istres",
                    "RC Grasse VS GFA Rumilly Vallières",
                    "AS Saint-Priest VS SC Toulon",
                    "US Lusitanos Saint-Maur VS Hyères FC",
                    "GOAL FC VS Andrézieux-Bouthéon FC",
                    "US Créteil Lusitanos VS FC Limonest DSD",
                    "FC 93 VS EFC Fréjus Saint-Raphaël"
                ],
                "J30": [
                    "AS Cannes VS US Lusitanos Saint-Maur",
                    "FC Limonest DSD VS Nîmes Olympique",
                    "Andrézieux-Bouthéon FC VS FC Rousset Sainte-Victoire",
                    "EFC Fréjus Saint-Raphaël VS RC Grasse",
                    "GFA Rumilly Vallières VS US Créteil Lusitanos",
                    "Hyères FC VS AS Saint-Priest",
                    "FC Istres VS GOAL FC",
                    "SC Toulon VS FC 93"
                ]
            }
        }
        ...
        with tab3:
            ...
            afficher_rating(scores_df, joueur_scores, kpis_panel, poste=poste, note_max=100)
        ...
        with tab7:
            if poste != 'Gardien':
                ...
                df_précision = df[df['Joueur + Information'] == joueur][[
                    'Passes arrière précises, %',
                ...
                df_précision = df[df['Joueur + Information'] == joueur][[
                    'Passes courtes / moyennes précises, %',
                    'Longues passes précises, %'
                ]].round(1)
        ...
        type_de_comparaison = st.radio("Sélectionnez le type de comparaison", ["Radar technique", "Radar physique", "KPI", "Statistiques avancées"])
